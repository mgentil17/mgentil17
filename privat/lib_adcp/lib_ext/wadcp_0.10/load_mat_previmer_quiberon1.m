function load_mat_previmer_quiberon1
warning off all
...
file=xlsread('PMD4_Quiberon_1_calibration_houle.xlsx');
time_houle=datenum(file(:,1),file(:,2),file(:,3),file(:,4),file(:,5),file(:,6));
HS=file(:,7);


%%
% charger fichier NTUSB 2009
file=xlsread('PMD4_Quiberon_1_NTUSB457_fond_T2.xlsx');
time_NTUSB=datenum(file(:,3),file(:,2),file(:,1))+file(:,4);
NTUSB=file(:,6); 

%%
% charger fichier FLNTUSB 2010
file=xlsread('PMD4_Quiberon_1_FLNTU1652_fond.xlsx');
time_FLNTUSB=datenum(file(:,3),file(:,2),file(:,1))+file(:,4);
FLNTUSB=file(:,8)/1000; % passage en g/l 


%%
% charger fichier MS5 46023
file=xlsread('PMD4_Quiberon_1_MS5_surface_SN46023_all.xlsx');
time_MS5_a=datenum(file(:,3),file(:,2),file(:,1))+file(:,4);
MS5_a=file(:,9);

%%
% charger fichier MS5 45800
file=xlsread('PMD4_Quiberon_1_MS5_surface_SN45800.xlsx');
time_MS5_b=datenum(file(:,3),file(:,2),file(:,1))+file(:,4);
MS5_b=file(:,9);
...
subplot(3,1,2), hold on
plot(time_MS5_a,MS5_a,'Color',icolor(:,3)), title('Concentration en MES (g/l)','FontSize',fsize), ylabel('Concentration en MES (g/l)','FontSize',fsize);axis([tm_min tm_max 0 Mglmax]);
plot(time_MS5_b,MS5_b,'Color',icolor(:,3))
...
plot(time_NTUSB,NTUSB,'Color',icolor(:,1)), title('Concentration en MES (g/l)','FontSize',fsize), ylabel('Concentration en MES (g/l)','FontSize',fsize);axis([tm_min tm_max 0 Mglmax]);
plot(time_FLNTUSB,FLNTUSB,'Color',icolor(:,1))
