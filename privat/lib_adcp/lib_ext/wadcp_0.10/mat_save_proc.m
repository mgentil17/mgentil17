...
global nbens theta WN WS WB IMM ens_int f a_t ouv Kc EC0 B SL0 Lt binpos inst_mode binpos_bt Mgl BI yrange xrange mode_exp fidlog time_Mgl
...
fileout_name=strcat(name,'_proc.mat');

else
    fileout_path=filein_path;
    fileout_name=strcat(filein,'_proc.mat');
end
cd(fileout_path)
...
if inst_mode==1
    data.binpos_bt.obj='Bin cell position from the bed for BT mode'; data.binpos.unit='m'; data.binpos.val=binpos_bt;
    data.yrange_bt.obj='Bin cell position from the bed for BT : BI and Mgl data';data_yrange.unit='m'; data.yrange.val=yrange(end)-yrange;

end
    data.binpos.obj='Bin cell position from the sensor'; data.binpos.unit='m'; data.binpos.val=binpos;
    data.yrange.obj='Bin cell position from the sensor for BI and Mgl data';data_yrange.unit='m'; data.yrange.val=yrange;    
...
data.time_Mgl.obj='Subtime for Mgl and BI - Matlab serial time for ensembles'; data.time_Mgl.unit='Days since 1900/1/1'; data.time_Mgl.val=time_Mgl;

data.BI.obj='Backscatter Index'; data.BI.unit='dB'; data.BI.val=BI';
data.Mgl.obj='SPM mass concentratio'; data.Mgl.unit='g/l'; data.Mgl.val=Mgl';
