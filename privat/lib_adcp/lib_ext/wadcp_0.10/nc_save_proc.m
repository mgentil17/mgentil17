...
global Batt Tb_adcp Depth_adcp Sb_adcp yrange xrange binpos binpos_bt inst_mode mode_exp 
global nbens theta WN WS WB IMM ens_int f a_t ouv Kc EC0 B SL0 Lt  Mgl BI fidlog


cd(filein_path);


%% tmp pour PAD : sauvegarde en txt de time/binpos/BI
% 
% 
% display('Saving processed data in TXT files')
% save('time_adcp.txt','time','-ASCII','-Double','-Tabs');
% save('binpos_adcp.txt','binpos','-ASCII','-Double','-Tabs');
% save('BI_average_adcp.txt','BI','-ASCII','-Double','-Tabs');
% display('Saving processed data in NETCDF files ? ')
% pause

%%


%[fileout_name,fileout_path]=uiputfile('*.nc','Name of the netcdf file where to save processed data:',filein_name(1:end-3));
...
ncquiet
fileout_name=strcat(filein,'_proc.nc');
ncout=netcdf(fileout_name,'clobber');

ncout.description='WADCP 0.5 ADP results';
...
ncout{'yrange'}={'yrange'};  ncout{'yrange'}.units='Bin cell position from the sensor for BI and Mgl data (m)';ncout{'yrange'}(:)=yrange;
ncout{'yrange_bt'}={'yrange_bt'};  ncout{'yrange_bt'}.units='Bin cell position from the bed for BI and Mgl data (m)';ncout{'yrange_bt'}(:)=yrange(end)-yrange;
else
...
fprintf(fidlog,'output file : %s\n',fileout_name)
fprintf(fidlog,'\n')
fprintf(fidlog,'PROCESSING COMPLETED')
fclose(fidlog);
