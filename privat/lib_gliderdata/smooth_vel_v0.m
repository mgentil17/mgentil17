function [dm_vel_filt, sm_vel_filt, DAC_cur] = smooth_vel_v0(dm_vel,sm_vel,constrain,max_dpth,nwinc,nwinr,plot_out,dpath)
...
names = fieldnames(dm_vel);                       % number of sections

for ii = 1:length(names)
    dm_cur = dm_vel.(names{ii});
    ...
    % Currents from direct method
    % Initialize matrix
    u_dm = NaN(max_dpth,length(sm_cur));                     % east-west component
    v_dm = NaN(max_dpth,length(sm_cur));                     % north-south component
    ustd_dm = NaN(max_dpth,length(sm_cur));                  % uncertainties
    vstd_dm = NaN(max_dpth,length(sm_cur));                  % uncertainties
    z_dm = NaN(max_dpth,length(sm_cur));                     % depth

    % Currents from shear method
    % Initialize matrix
    u_sm = NaN(max_dpth,length(sm_cur));
    v_sm = NaN(max_dpth,length(sm_cur));
    ustd_sm = NaN(max_dpth,length(sm_cur));                  
    vstd_sm = NaN(max_dpth,length(sm_cur));                  
    z_sm = NaN(max_dpth,length(sm_cur));
...
% %     for j = 1:length(dm_cur)
% %         if ~any(dm_cur{j}.uoffset) || ~any(dm_cur{j}.voffset)
% %             dm_cur{j}.uc_adjust = NaN(size(dm_cur{j}.vx));
% %             dm_cur{j}.vc_adjust = NaN(size(dm_cur{j}.vx));
% %         end
% %     end

    % Convert cell to matrix
    struct_dm = cell2mat(dm_cur);                       % direct currents data
    ...
    for j = 1:length(sm_cur)
        ...
        % Direct method
        u_dm(1:length(struct_dm(j).uc_adjust),j) = struct_dm(j).uc_adjust;
        v_dm(1:length(struct_dm(j).vc_adjust),j) = struct_dm(j).vc_adjust;
        ustd_dm(1:length(struct_dm(j).uc_adjust_std),j) = struct_dm(j).uc_adjust_std;
        vstd_dm(1:length(struct_dm(j).vc_adjust_std),j) = struct_dm(j).vc_adjust_std;
        z_dm(1:length(struct_dm(j).edges),j) = struct_dm(j).edges;
        ...
        % Backscatter index
        BI_dm(1:length(struct_dm(j).BI_mean),j) = struct_dm(j).BI_mean;
        BI_sm(1:length(struct_sm(j).BI_mean),j) = struct_sm(j).BI_mean;
        BIstd_dm(1:length(struct_dm(j).BI_std),j) = struct_dm(j).BI_std;
    ...
    %======================================================================
    % Direct method
    u_dm_filt = smooth2a(u_dm,nwinc,nwinr);
    v_dm_filt = smooth2a(v_dm,nwinc,nwinr);
    ustd_dm_filt = smooth2a(ustd_dm,nwinc,nwinr);
    vstd_dm_filt = smooth2a(vstd_dm,nwinc,nwinr);
    ...
    % Backscatter index
    BI_dm_filt = smooth2a(BI_dm,nwinc,nwinr);
    BI_sm_filt = smooth2a(BI_sm,nwinc,nwinr);
    BIstd_dm_filt = smooth2a(BIstd_dm,nwinc,nwinr);
    ...
    dm_vel_filt.(names{ii}).time = time;
    dm_vel_filt.(names{ii}).dpth = z_dm;
    dm_vel_filt.(names{ii}).u = u_dm_filt;
    dm_vel_filt.(names{ii}).v = v_dm_filt;
    dm_vel_filt.(names{ii}).ustd = ustd_dm_filt;
    dm_vel_filt.(names{ii}).vstd = vstd_dm_filt;
    dm_vel_filt.(names{ii}).cst = cst;
    dm_vel_filt.(names{ii}).BI = BI_dm_filt;
    dm_vel_filt.(names{ii}).BIstd = BIstd_dm_filt;
